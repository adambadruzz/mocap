...
class UpdateProfileView extends StatelessWidget {
  final UpdateProfileViewModel viewModel = Get.put(UpdateProfileViewModel());
  ...
  late Rx<Timestamp?> _selectedDate;
                          ...
                          children: [
                            Text(
                              DateFormat('dd MMM yyyy').format(
                                _selectedDate.value?.toDate() ?? DateTime.now(),
                              ),
  ...
  Future<void> _fetchCurrentUser() async {
    ...
    final userDetails = await viewModel.getCurrentUser();
    ...
    _selectedDate.value = userDetails['dob'] as Timestamp?;
  }
  ...
  Future<void> _updateProfile() async {
    ...
    if (_selectedImage.value != null) {
      ...
      final oldPhotoUrl = await viewModel.getProfilePhotoUrl();
      if (oldPhotoUrl != null) {
        await FirebaseStorage.instance.refFromURL(oldPhotoUrl).delete();
      }

      // Unggah foto terbaru ke Firebase Storage
      final photoUrl = await viewModel.uploadProfilePhoto(_selectedImage.value!);
      updatedUserDetails['photourl'] = photoUrl;
    }

    await viewModel.updateProfile(updatedUserDetails);
  ...
  Future<void> _selectDate(BuildContext context) async {
    final DateTime? pickedDate = await showDatePicker(
      context: context,
      initialDate: _selectedDate.value?.toDate() ?? DateTime.now(),
      firstDate: DateTime(1900),
      lastDate: DateTime.now(),
    );
    if (pickedDate != null && pickedDate != _selectedDate.value?.toDate()) {
      _selectedDate.value = Timestamp.fromDate(pickedDate);
    }
  }
